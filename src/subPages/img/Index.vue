  ...
  <page-wraper>
    <view class="page-img">
      <demo-group title="基本用法">
        <demo-group-item title="基本用法"> <up-image :height="100" :src="imgURL" :width="100" /> </demo-group-item>
      </demo-group>

      <demo-group title="延时加载">
        <demo-group-item title="延时加载"> <up-image :delay="2000" :height="100" :src="imgURL" :width="100" /> </demo-group-item>
      </demo-group>

      <demo-group title="预加载占位图">
        <demo-group-item title="预加载占位图">
          <up-image :delay="2000" :height="120" :placeholder-src="imgURL + '?x-oss-process=image/resize,p_50'" :src="imgURL" :width="120" />
        </demo-group-item>
      </demo-group>
        ...
        <demo-group-item title="圆角">
          <view class="page-img__grid">
            <view class="page-img__col" v-for="mode in modes.slice(0, 6)" :key="`radius-${mode}`">
              <up-image height="27vw" width="100%" :mode="mode" :radius="8" :src="imgURL" />
        ...
        <demo-group-item title="圆形">
          <view class="page-img__grid">
            <view class="page-img__col" v-for="mode in modes.slice(0, 6)" :key="`round-${mode}`">
              <up-image height="27vw" round width="100%" :mode="mode" :src="imgURL" />
              <view class="page-img__text">{{ mode }}</view>
            </view>
          </view>
        </demo-group-item>
        ...
        <demo-group-item title="填充模式">
          <view class="page-img__grid">
            <view class="page-img__col" v-for="mode in modes" :key="mode">
              <up-image height="27vw" width="100%" :mode="mode" :src="imgURL" />
              <view class="page-img__text">{{ mode }}</view>
            </view>
          </view>
        </demo-group-item>
      ...
      <demo-group title="图片预览">
        <demo-group-item title="图片预览"> <up-image mode="widthFix" width="100%" :enable-preview="true" :src="imgURL" /> </demo-group-item>
      </demo-group>
        ...
        <demo-group-item title="加载失败">
          <view class="page-img__grid page-img__grid--two">
            <view class="page-img__col">
              <up-image height="27vw" src="https://www.invalid-url.com/a.jpg" width="100%" />
...
.page-img {
  ...
  &__text { margin-top: 8rpx; text-align: center; font-size: 24rpx; color: var(--up-text-secondary); }
