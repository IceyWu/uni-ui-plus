...
export function usePopover(visibleArrow = true) {
  ...
  function init(placement: Placement, visibleArrow: boolean, selector: string) {
    if (visibleArrow) {
      const arrowClassArr = [
        `wd-${selector}__arrow`,
  ...
  function control(placement: Placement, offset: number | number[] | Record<'x' | 'y', number>) {
    ...
    if (Array.isArray(offset)) {
      ...
      offsetX = (verticalX - 17 > 0 ? 0 : verticalX - 25) + (offset as Record<'x' | 'y', number>).x
      offsetY = (horizontalY - 17 > 0 ? 0 : horizontalY - 25) + (offset as Record<'x' | 'y', number>).y
    } else {
