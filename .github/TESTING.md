...
本文档提供了 uni-ui-plus 组件库的测试策略、工作流程和最佳实践指南。
...
uni-ui-plus 使用 GitHub Actions 自动化测试流程，主要工作流文件是 `.github/workflows/component-testing.yml`。
...
4. 可以选择性地指定要测试的组件名称（例如：up-button）
...
import WdComponent from '../../src/uni_modules/uni-ui-plus/components/up-component/up-component.vue'

describe('WdComponent', () => {
  // 测试基本渲染
  test('基本渲染', () => {
    const wrapper = mount(WdComponent)
    expect(wrapper.classes()).toContain('up-component')
  })
...
test('基本渲染', () => {
  ...
  expect(wrapper.classes()).toContain('up-button')
})
```
...
test('按钮类型', () => {
  ...
  expect(wrapper.classes()).toContain('up-button--primary')
})
```
...
// 不推荐（除非必要）
const MockButton = {
  template: '<button class="up-button"></button>'
}
